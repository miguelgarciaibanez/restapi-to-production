...
describe('save', () => {
    it('Should create user', async () => {
        ...
        //TODO TRY TO MAKE IT WITH USER INSTEAD OF FETCHED
...
describe('toJSON', () => {
    it('should return valid JSON', async () => {
        ...
        const user = new User({ email: email, password: password, name: name })
        await user.save()
        expect(user.toJSON()).toEqual({ email: email, name: name, created: expect.any(Number), password: expect.any(String) })
    })
})